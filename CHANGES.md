...
- **The batch size of the scheduled tasks has an administration page**
  The tasks walk a course's enrolled users in pages, and the size of those pages decides how much work a run holds at once. It could only be changed from code or from the site's configuration file, which left the person responsible for cron unable to see it, let alone tune it. It is now a setting under Local plugins, with the same default of 500 the tasks already used, and the description says what raising or lowering it costs.- **The engine switching a rule off is recorded in the log**
