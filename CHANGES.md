...
- **A reinforcement activity generated by AI no longer moves anybody else's grade**
  An activity generated for one student was still a gradebook column for the whole course: empty for everyone it was hidden from, and under "Lowest grade" that empty column took their course total to zero. When the action is configured it now asks one question - does this activity's grade count for the student who receives it, or for nobody - and either way **every other student the gradebook counts is excluded from that column one by one**. That is the only mechanism that holds under all nine of Moodle's aggregation methods, because core drops excluded grades before it picks a method; grade categories and weights cannot do it at all, having no per-student dimension.

  Choosing **no** leaves the column counting for nobody: the activity may still be graded and the teacher can use that grade as feedback, but it moves no total anywhere. Choosing **yes** leaves it counting for the one student it was generated for, as an ordinary activity - pending work until they do it, a grade once they have. A student who enrols later, whose suspended enrolment is reactivated, or who is given a gradebook role after being enrolled, is excluded as they arrive.

  One limit worth knowing: the column has to stay directly under the course. Inside a grade category the exclusions stop protecting the students they were written for, because the category comes out empty for them and an empty category counts as a zero one level up - so the group loses points again, measured at 80% falling to 40% under Mean and 0% under Lowest grade. This is Moodle's own behaviour for any excluded grade filed into a category, with or without this plugin, and the plugin does not try to undo it: an earlier attempt to move the activity back and delete the emptied category was found to orphan the grade item and take every gradebook page in the course down with it.

  **Site administrators, note:** rules saved before this version carry no stored choice, and the upgrade reads them as "counts for nobody" - the option that cannot cost anybody points - rather than reproducing the previous behaviour.
