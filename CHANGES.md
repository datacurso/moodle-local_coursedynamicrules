...
## Added
- **The batch size of the scheduled tasks has an administration page**
  The tasks walk a course's enrolled users in pages, and the size of those pages decides how much work a run holds at once. It could only be changed from code or from the site's configuration file, which left the person responsible for cron unable to see it, let alone tune it. It is now a setting under Local plugins, with the same default of 500 the tasks already used, and the description says what raising or lowering it costs.
...
- **The scheduled tasks no longer load a whole course's users into memory at once**
  The three enrolment-wide tasks - two of which run every minute - fetched every enrolled user of every course with an active rule as one array of complete user records, although the rule engine reads a single field of each: the id. On a large course that array was held for the whole run, on every run. The tasks now walk the course in pages of ids, keyed on the last id seen rather than on an offset, so a student unenrolled while the task runs cannot make the next page skip somebody else. The page size is the existing `taskbatchsize` value (500 by default), which until now only drove the over-threshold notice and had no page in Site administration; this release gives it one, under Local plugins. Users are fetched only for rules that are actually due, where before they were loaded for every active rule, and are now walked in id order rather than by name. In the two tasks that emit it, the over-threshold notice still reports the course's size. The walk is live rather than a snapshot: a student unenrolled ahead of the page being read is not evaluated by that run, and a student enrolled during it may be, where before both waited for the next run.
- **A stray byte no longer sends the student's name to the AI service**
  Before the AI action sends its request, it replaces the student's name in the text with a placeholder. That replacement runs a Unicode-aware search, which the engine refuses outright when the text carries a byte that is not valid UTF-8 - which happens when a name or an instruction reaches Moodle from a system that encoded it differently, through an import or an account synchronisation. On that refusal the action kept the original text and sent it, so the request that left the site still named the student, with nothing reporting a problem. The text is now repaired before the search, by replacing the offending byte rather than deleting it: deleting it would glue a first and last name into a single word that no longer matches a name standing on its own, and the full name would travel intact. A name is looked for both as it is stored and as it would have been typed, so a damaged profile does not hide a correctly written name in the text. Text that is already valid is sent exactly as written, and what is put back into the generated activity is always storable. If the replacement still cannot be performed the generation is refused and nothing is sent, which is reported in the run's output. **Site administrators, note:** one case is not covered - a byte damaged in the MIDDLE of the name as written in the text leaves nothing to match, and that occurrence is still sent. And a refusal consumes that rule's run like any other failure, so a one-shot rule that refuses does not try again.
...
- **The privacy declaration names the personal fields this plugin hands to the AI client**
  The plugin declares, through the Privacy API, the data it hands to its AI client when the create-AI-activity action runs, and that declaration had drifted away from the request: it named three fields the action no longer hands over - a course id, a course URL and a `prompt` - and omitted three it does, among them the language taken from the user's own preference. Only the user id was right. The declaration had been written against an earlier endpoint and was never revisited when the action moved to the current one, and nothing failed in the meantime because the only test covering it checked that each declared name resolved to a translated description, never that the field existed. The action hands seven keys to the client: the four containing personal or contextual data are now declared, while the test records the remaining three as operational controls. The test fixes the seven-key seam as an explicit contract and compares the declaration against the request in both directions, so adding, removing or renaming a key requires an explicit contract update; it does not establish a legal classification or claim to inspect fields a dependency may add later during transport. The description of the instructions field also states what it can carry, which is more than the sentence it replaced admitted: the prompt configured on the action, the course's own AI context prompt when the course has one - prepended whether or not the prompt uses any marker - and, where the markers are used, the course name and course URL. It states that the student's name is replaced before the request leaves, and that this is pseudonymisation rather than anonymisation, since the same request carries the user id and the replacement is reversed on the answer. One thing the corrected declaration does not do yet is appear: the plugin privacy registry only renders the fields of a component core counts as compliant, and a component with a metadata provider alone is not (see Known limitations), so the declaration is right in code and invisible on that screen until the provider is completed.

## Known limitations
- **The privacy provider declares data but does not yet export or erase it**
  The provider implements the metadata contract only. Two consequences follow, and both are stated exactly because an earlier draft of this entry got one of them wrong. First, core does not count the component as compliant - the Privacy API requires a data provider as well - so the plugin privacy registry lists it as non-compliant and renders none of its declared fields, including the external AI transfer corrected in this release. Second, an approved export or deletion request runs none of the provider's own code, because there is none to run; it is NOT true that such a request runs nothing of this plugin. An approved deletion ends by deleting the account, and that fires the user-deletion observer described under 1.8.4, which removes the user's id from the restrictions the enable-activity action manages - and from those only. The create-AI-activity action writes its user id into an activity's access restrictions with no ownership marker at all, in the same shape a teacher's own hidden single-student restriction takes, so nothing can tell the two apart after the fact: on the reference site on 2026-09-11, 11 of the 18 activities carrying a user restriction had no marker. Export and deletion therefore remain open work, and the measurement fixes its shape: give the AI action's node an identity going forward, operate only on restrictions whose ownership the plugin can prove, and never guess about the historical unmarked ones. Separately, the shared HTTP client in aiprovider_datacurso adds five fields of its own to every request it sends - a site identifier, the site address, a timezone, a language and the user id. Those belong to that plugin and it declares them, as a distinct external location, from its release 1.5.1 (2026090700) onward; release 1.5.0 did not. This plugin's declared dependency is therefore raised to that build, so a site cannot satisfy it with a provider that leaves them undeclared.
...
- **Two actions can no longer open the same activity**
  Each enable-activity action writes its own gate into the activity's restrictions, and Moodle requires EVERY gate to have opened it for a student: a second gate, which is empty until its own rule runs, therefore closed the activity for the students the first action had already opened it for, from the moment the form was saved. The form now refuses an activity another action already opens, names it, and says so. Two exceptions, both deliberate: an action keeps whatever it already gates, so a pair created by an earlier version stays editable on both sides, and a gate carrying no owner mark is invisible to this check - which covers a gate written before 1.8.2 and also the one the create-AI-activity action writes today (see Known limitations). **Site administrators, note:** where such a pair exists, releasing the activity from one of the two actions is only possible while that action's rule has never been activated; a sealed rule cannot give it up.
...
- **Two rules opening the same activity**
  The form refuses a second action on one activity (see Changed), but three cases slip past it. A gate written before 1.8.2 carries no owner mark, so on a site upgraded from those versions a new action can still be pointed at an activity an old one opens, and the students the old one had opened it for lose it. The same is true on a site installed fresh, because the create-AI-activity action restricts the activity it generates with a gate it does not mark either: the refusal is told nothing about that activity, so an enable-activity action can be pointed at it without a warning, and the student the AI activity was generated for - the only one who could open it - stops being able to, since the second gate is empty until its own rule runs. This is measured rather than reasoned: the same check correctly names a normally gated activity in the same course. And the refusal lives on the form: any caller that saves an action without it - a course restore, a script - can still create the pair. Where a pair exists, a student sees the activity only once BOTH actions have opened it for them, and each action remembers how visible the activity was when it took it over and restores that when it is deleted, so the one deleted last decides.
- **Privacy exports, and the restriction on an AI-generated activity**
  The plugin's privacy provider declares metadata only, so a subject access request exports none of the ids a rule wrote into activity restrictions, and the privacy tool's own data-deletion pass does not touch them either; a deletion request does end by deleting the account, which runs the cleanup above. The create-AI-activity action also restricts the activity it generates to the student it was generated for, in a node it neither marks nor records, so that id is out of the cleanup's reach and stays after the student is deleted - and, for the same reason, out of reach of the refusal that stops two actions closing one activity (see above). A user restriction a teacher added by hand keeps a deleted user's id, as it always has in Moodle.
